"""
Cherwell District Council - Banbury HMO Data Source (Database Version)
"""
...
class CherwellBanburyHMOMapData(BaseHMODataSource):
    """
    Cherwell District Council - Banbury HMO Data Source
    Reads data from the HMORegistry database table
    """
    
    def __init__(self):
        super().__init__('cherwell_banbury')
        self.display_name = "Banbury (Cherwell District)"
    ...
    def load_raw_data(self) -> List[Dict]:
        ...
        try:
            records = db.query(HMORegistry).filter(
                HMORegistry.city == 'cherwell_banbury'
            ).all()
            
            logger.info(f"Loaded {len(records)} Banbury records from database")
            return [self._database_record_to_dict(record) for record in records]
            
        except Exception as e:
            logger.error(f"Error loading Banbury data: {e}")
            return []
        finally:
            db.close()
    ...
    def _transform_record(self, record: Dict) -> Dict:
        ...
        return {
            'id': f"cherwell_banbury_{record.get('case_number', record.get('id', ''))}",
            'case_number': record.get('case_number', ''),
            'source': 'cherwell_district_council',
            'city': 'cherwell_banbury',
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error getting Banbury statistics: {e}")
            return self._empty_stats()
        finally:
            db.close()