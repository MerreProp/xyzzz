"""
Cherwell District Council - Bicester HMO Data Source (Database Version)
"""
...
class CherwellBicesterHMOMapData(BaseHMODataSource):
    """
    Cherwell District Council - Bicester HMO Data Source
    Reads data from the HMORegistry database table
    """
    
    def __init__(self):
        super().__init__('cherwell_bicester')  # ✅ CORRECT CITY
        self.display_name = "Bicester (Cherwell District)"  # ✅ CORRECT NAME
    ...
    def load_raw_data(self) -> List[Dict]:
        ...
        try:
            records = db.query(HMORegistry).filter(
                HMORegistry.city == 'cherwell_bicester'
            ).all()
            
            logger.info(f"Loaded {len(records)} Bicester records from database")
            return [self._database_record_to_dict(record) for record in records]
            
        except Exception as e:
            logger.error(f"Error loading Bicester data: {e}")
            return []
        finally:
            db.close()
    ...
    def _transform_record(self, record: Dict) -> Dict:
        ...
        return {
            'id': f"cherwell_bicester_{record.get('case_number', record.get('id', ''))}",
            'case_number': record.get('case_number', ''),
            'source': 'cherwell_district_council',
            'city': 'cherwell_bicester',
    ...
    def get_statistics(self) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error getting Bicester statistics: {e}")
            return self._empty_stats()
        finally:
            db.close()